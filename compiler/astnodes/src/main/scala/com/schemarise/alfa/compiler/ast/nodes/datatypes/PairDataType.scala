...
class PairDataType(location: IToken = TokenImpl.empty, l: DataType, r: DataType)
  extends EnclosingDataType(location, Enclosed.pair, l) with IPairDataType {
  private var _left = l
  ...
  override def componentType = throw new com.schemarise.alfa.compiler.AlfaInternalException("Component type not accessible for pair<L,R>")
  ...
  override def isUnmodifiedAssignableFrom(other: IAssignable) = {
    if (other.isInstanceOf[PairDataType] &&
      l.isAssignableFrom(other.asInstanceOf[PairDataType].left) &&
      r.isAssignableFrom(other.asInstanceOf[PairDataType].right)
    )
      true
