...
class RefactorExporterTest extends AlfaFunSuite {
  ...
  test("Test group refactor") {
    val cua = TestCompiler.compileScriptOnly(super.testResourcesContent("alfa/testModels1.alfa"))

    val tgtDir2 = targetGeneratedTestSourcesPath("refactor/group-refactor/")
    val m2 = new util.HashMap[String, Object]()
    m2.put(RefactorExporter.Namespace, "refactor.test1")
    m2.put(RefactorExporter.OnlyIncludeFields, "id,user_id,fname,lname")
    m2.put(RefactorExporter.AttribSizeThreshold, "2")
  ...
  test("Test OnlyIncludeTypes refactor") {
    val cua = TestCompiler.compileScriptOnly(super.testResourcesContent("alfa/testModels1.alfa"))

    val tgtDir2 = targetGeneratedTestSourcesPath("refactor/only-include-types-refactor/")
    val m2 = new util.HashMap[String, Object]()
    m2.put(RefactorExporter.Namespace, "refactor.test1")
    m2.put(RefactorExporter.OnlyIncludeTypes, "refactor.test1.Review,refactor.test1.Order")

    val rst2 = new RefactorExporter(AlfaExporterParams(new StdoutLogger(), tgtDir2, cua, m2))
    rst2.exportSchema()

    val refactoredFiles = VFS.listFileForExtension(tgtDir2, "alfa")

    assert(refactoredFiles.size == 11)

    val newBase = VFS.read(tgtDir2.resolve("refactor.test1.Base1.alfa"))

    assert( equalsIgnoringWhitespace(newBase,
      """
        |// Generated by ALFA Refactor
        |
        |trait refactor.test1.Base1 {
        |  created_at : datetime
        |  id : uuid
        |  user_id : uuid
        |}
