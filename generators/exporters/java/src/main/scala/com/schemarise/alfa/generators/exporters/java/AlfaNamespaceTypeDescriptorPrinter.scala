...
/**
 * ALFA namespace has its own Java model printer that does not rely on JSONs from META-INF that can cause loading cycles
 * TODO remove duplication between user type and alfa descriptor printer
 */
class AlfaNamespaceTypeDescriptorPrinter(logger: ILogger, outputDir: Path, cua: ICompilationUnitArtifact, compilerToRt: CompilerToRuntimeTypes)
  ...
  private def supplier(t: IUdtBaseNode, f: IField): String = {
    ...
    val fn = f.name
    val fieldTypeVar = n + "Type"

    val code = fieldDataType match {
      case d: ITypeParameterDataType =>
        ""
      case _ =>
        s"""${nestedSupplier(fieldDataType, fn, 1, "", fieldTypeVar + "")}
           |        private java.util.function.BiConsumer< $parentJavaType, com.schemarise.alfa.runtime.DataConsumer > ${n}Supplier = ( p, consumer ) -> {
  ...
  private def consumer(udt: IUdtBaseNode, f: IField): String = {
    ...
    val fieldTypeVar = jFieldName + "Type"
    val fieldDataType = f.dataType
    ...
    val code = fieldDataType match {
      case t: TypeParameterDataType =>
        ""
      case _ =>
        s"""${nestedConsumer(fieldDataType, rawName, 1, "", fieldTypeVar + "")}
           |        java.util.function.BiConsumer< ${builderImplClz(udt)}, com.schemarise.alfa.runtime.DataSupplier > ${jFieldName}Consumer = ( builder, supplier ) -> {
  ...
  def buildDeclAndDataType(t: IDataType): (String, String) = {
    ...
    val json = com.schemarise.alfa.runtime.Alfa.jsonCodec().toJsonString(rtType)
    ...
    t match {
      ...
      case _ =>
        throw new com.schemarise.alfa.runtime.AlfaRuntimeException("Unsupported in java codegen " + t + " " + t.getClass.getName);
    }
  }
  ...
  def print(udt: IUdtBaseNode): String = {
    ...
    val fieldIdConsts = accessorFields.values.zipWithIndex.map(f => {
      s"""        public final static short FIELD_ID_${validLangIdentifier(f._1.name.toUpperCase, LangKeywords.javaKeywords)} = ${f._2};
         |        public final static String ${fieldName(f._1)} = "${f._1.name}";
         |""".stripMargin
    }).mkString("\n")
    ...
    val maskToName = accessorFields.values.zipWithIndex.map(f => {
      s"""
         |                case ${fieldId(f._1)} : return FIELD_${f._1.name.toUpperCase};""".stripMargin
    }).mkString("")
    ...
    val assertsMapBuild = udt.allAsserts.map(f => {
      ...
      val json = com.schemarise.alfa.runtime.Alfa.jsonCodec().toJsonString(assertModel)
    ...
    val fieldMetas = accessorFields.map(f => {
      ...
      if (noConsumerSupplier || f._2.dataType.isInstanceOf[TypeParameterDataType])
        s"""        public com.schemarise.alfa.runtime.FieldMeta $metaField = new com.schemarise.alfa.runtime.FieldMeta( java.util.Optional.empty(), java.util.Optional.empty(), java.util.Optional.empty(), java.util.Optional.empty(), $fType, ${fieldName(f._2)}, $fieldAnnotation );"""
      else
        s"""        public com.schemarise.alfa.runtime.FieldMeta $metaField = new com.schemarise.alfa.runtime.FieldMeta( java.util.Optional.of( $fSupp ), java.util.Optional.of( $fCons ), java.util.Optional.of( ${fSupp}Inner1 ), java.util.Optional.of( ${fCons}Inner1 ), $fType, ${fieldName(f._2)}, $fieldAnnotation );"""
    ...
    val kModel = if (udt.isInstanceOf[IEntity] && udt.asInstanceOf[IEntity].key.isDefined) {
      val k = udt.asInstanceOf[IEntity].key.get
    ...
    val mods = if (rtUdt.getModifiers.contains(ModifierType.Internal)) {
      s"add(schemarise.alfa.runtime.model.ModifierType.Internal);"
    }
    else {
      ""
    }
    ...
    val deps =
      if (depsOpt.isPresent) {
        "new java.util.HashSet( java.util.Arrays.asList(" + depsOpt.get().asScala.map(w => "\"" + w.getFullyQualifiedName + "\"").mkString(", ") + ") )"
      }
      else "java.util.Collections.emptySet()"
    ...
    s"""
       ...
       |
       |        private schemarise.alfa.runtime.model.UdtDataType _asUdtType_ = schemarise.alfa.runtime.model.UdtDataType.builder().setUdtType(schemarise.alfa.runtime.model.UdtMetaType.${udtType}Type).setFullyQualifiedName(TYPE_NAME).build();
       |$fieldIdConsts
       |$fieldDefns
       |$consumers
       |$suppliers
