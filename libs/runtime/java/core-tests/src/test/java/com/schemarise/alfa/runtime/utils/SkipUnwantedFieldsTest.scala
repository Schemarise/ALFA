...
import com.schemarise.alfa.runtime.{Alfa, JsonCodec, ValidationCollectingListener}
...
class SkipUnwantedFieldsTest extends AnyFunSuite {
  ...
  def decode(s: String) = {
    val jc = JsonCodecConfig.builder().
      setAssignableToClass(classOf[SkipUnknownField]).
